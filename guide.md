...
Version 0.7.6 (draft), February 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
There are some crypto currencies with privacy/anonymity in mind like Monero but even those have some and warnings to consider[^119]'[^120].

Even if you use Mixers or Tumblers[^121] (services that specialize in "anonymizing" crypto currencies by "mixing them"), keep in mind this is only obfuscation[^122] and not actual anonymity[^123]. Not only are they only obfuscation but they could also put you in trouble as you might end up exchanging your crypto against "dirty" crypto that was used in various questionable contexts[^124].
...
So, you're using Tor Browser or Brave Browser over Tor. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^125] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^126] case in 2013 where the FBI inserted malware[^127] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^128] hack that breached several US government institutions by inserting malware into an official software update server.

In some countries, Malware is just mandatory and/or distributed by the state itself. This is the case for instance in China with WeChat[^129] which can then be used in combination with other data for state surveillance[^130].
...
If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^138] or Coreboot[^209] if your laptop supports it.
...
This technology is also widely used in CAPTCHAS[^264] services to verify that you are "human" and can be used to fingerprint a user.
...
Even if you're not looking at the camera, they can still figure out who you are[^161], make out your emotions[^162], analyze your gait[^163] and probably guess your political affiliation[^164].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^165]'[^166]'[^167]'[^168]'[^169].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^170]) in their CCTV networks to look for "persons of interest"[^171]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^172] including possibly identifying ethnic minorities[^173]. A simple face recognition error by some algorithm can ruin your life[^174].
...
Same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^175].
...
-   Wear a facemask as they have been proven to defeat some face recognition technologies[^184].
...
Phishing[^185] is a social engineering[^186] type of attack where an adversary could try to extract information from you by pretending to be something/someone else.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they're encrypted[^187]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And last but not least here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^188]
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^189] or a national security letter[^190]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^191].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^192].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^193].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^194].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^195].

Some providers have implemented the use of a Warrant Canary[^196] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Last but not least, it's now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^197] and Omnisec story[^198]).
...
There are many advanced techniques that can be used by skilled adversaries[^199] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^200] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw>
...
-   If you absolutely want system wide plausible deniability[^220]'[^201] despite the risks[^202]'[^222].
...
-   Check some data yourself here on the Tor Project OONI[^203] (Open Observatory of Network Interference) website: <https://explorer.ooni.org/>
...
**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^211]**.**
...
This is the hardest part of the whole guide. It's a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting more and more limited due to various KYC type regulations[^204].
...
Some might be very efficient[^205] but many are gimmicky gadgets that offer no real protection[^206].
...
TAILS[^207] stands for **The Amnesic Incognito Live System**. It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^208]**.**
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm>) or System76 laptops (<https://system76.com/>) while using Coreboot[^209] (where Intel IME is disabled from factory).
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. However, if you're going for the Qubes Route consider Intel as they do not support AMD with their anti-evil-maid system[^210].
...
-   Enable BIOS/UEFI password and use a long passphrase[^211] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^212]. Keep it on if you intend to use Linux/Windows.
...
So, what is Secure Boot[^213]? In short, it's a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically, when the Operating Systems (or the Bootloader[^214]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^215] (with pictures).
...
This route will make extensive use of Virtual Machines[^216], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for anonymity depending on your threat model. This is because using Windows will allow us to use Plausible Deniability[^220] easily at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^221] but is the only (convenient) option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.

So, what is Plausible Deniability? It's the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^222].

A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^223]'[^224] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will come into play.
...
Cold Boot attacks[^227] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^228], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability, there have been some forensics studies[^229] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^230] and by the maintainer of Veracrypt[^231] so I wouldn't worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^233].

-   Windows Indexing keeping traces of the files present in your User folder by default[^234].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^235].
...
-   Gatekeeper[^236] and XProtect keeping track of your download history in a local database and file attributes.
...
Veracrypt[^237] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^238] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^239] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^240] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^241] Operations[^242]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^211] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^249] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^250] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^251] have not be completely fixed to this date[^252] and if you're using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This route will use Virtualization and Whonix[^253] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later on, you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^254] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^260], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^262] information).

For this purpose, I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^263] authentication with the authenticator feature.
...
Captcha[^264] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^265] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^266]) and Cloudflare (hCaptcha[^267]). hCaptcha is used on 15% of the internet by their own metrics[^268].

They're designed to separate bots from humans but in reality, are also used to deter anonymous and private users.

If you frequently use VPNs or Tor, you'll quickly encounter many captchas everywhere[^269]. Quite often when using Tor, even if you succeed in solving all the puzzles, you'll still be denied after solving the puzzles.

While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^270]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^271].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^272].
...
You will often experience several in a row (sometimes endless[^273]) and sometimes very difficult ones involving reading undecipherable characters or identifying various objects on endless pictures set. You'll also have more captchas if you use ad blocking system or if your account was flagged for any reason for using VPNs or Tor previously.
...
Most platforms (including the privacy-oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^275].
...
Browser and Device[^276] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^277]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^278] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^279].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^280] and unfortunately sometimes required.
...
This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^281]. Unfortunately, this method is now also sometimes being used on Facebook[^282] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^283].
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^284] instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^285]).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will definitely have less issues with captchas and online platforms[^277] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Always use TOTP 2FA (not SMS to prevent Sim Swapping attacks[^286] and to keep your identity working when your pre-paid card expires) using KeePassXC when available to secure your logins to various platforms.

-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^211]**.**
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^287].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^288].
...
| Discord     | No              | No                 | Yes                 | Yes             | Yes             | No           | No            | No                | No                | Medium                 |
...
| Google      | No              | Maybe              | Maybe               | Yes             | Yes             | Yes          | No            | No                | Maybe             | Medium                 |
| Instagram   | Yes             | Maybe              | Yes                 | Yes             | Yes             | Yes          | No            | Maybe             | Maybe             | Medium                 |
| LinkedIn    | Yes             | Yes                | Yes                 | Yes             | Yes             | Yes          | Maybe         | Maybe             | Maybe             | Hard                   |
| Microsoft   | Yes             | Maybe              | Maybe               | Yes             | Yes             | Yes          | No            | No                | No                | Medium                 |
| ProtonMail  | No              | Maybe              | Maybe               | Yes             | Yes             | Yes          | No            | No                | No                | Medium                 |
| Reddit      | No              | No                 | No                  | Yes             | Yes             | No           | No            | No                | No                | Easy                   |
| Telegram    | No              | Yes                | No                  | Yes             | Yes             | No           | No            | No                | No                | Easy                   |
| Twitter     | No              | Maybe              | Yes                 | Yes             | Yes             | Yes          | No            | No                | Maybe             | Medium                 |
...
-   Can you create accounts through Tor? Yes, but it's very difficult and their onion address[^289] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in."
...
If your account gets suspended [^292] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^293] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^294] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^295]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^296]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
This time you will need to submit an appeal using a form[^297], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using at least seven proxies[^298] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
Instead, you should use better options (as recommend by privacytools.io[^299]'[^300]) such as:
...
If you have to do so, you should take some steps because you can't really trust any organization to protect your anonymity[^301]:
...
For this, I strongly recommend the use of SecureDrop[^302] (<https://securedrop.org/>) which is an open-source project from the Freedom of the Press foundation.
...
So, the fact is that it's very unlikely[^309]'[^310] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it's not completely impossible either[^311]'[^312]'[^313] if they're fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^314] for 11\$ and use it to erase any disk.
...
-   If your file system is APFS, you don't need to worry about Trim, it apparently happens asynchronously as the OS writes data[^320] according to their own documentation.
...
Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^321]. So, you're left with using other tools:
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^322] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It's however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^323] and available on TAILS by default[^324].
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^240]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^233] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again, you shouldn't do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally, I would just pick Normal Deletion + Trim (Trim itself should be enough[^244]). Secure Deletion with Trim[^241] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^325] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^326] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^327] and Secure Communication Obfuscation[^328] through Steganography[^329] and/or Kleptography[^330] and has many names such as Koalang[^331] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
-   **Remember to use passphrases instead of passwords and use a different one for each service**[^211]**.**
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> [^332]
...
As written earlier in this guide and as noted by Privacytools.io[^333], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   The Windows USN journal which stores plenty of information[^335].

-   The Windows System Resource Usage Monitor (SRUM)[^336].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^337].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^338]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
Some might be very efficient[^339] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It's pretty much the only way to be sure it's not only supported but actually works. Some utilities can tell you if it's supported or not like CrystalDiskInfo[^340] but won't actually check if it's working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives:]
...
According to Apple Documentation[^320], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
-   If you really have to take them with you, you could consider the use of a faraday cage[^341] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^342] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
-   Wearing gloves while manipulating anything to avoid leaving fingerprints[^343] and touch DNA[^344].
...
-   Don't lick the envelope or the stamps[^345] if you use them to avoid leaving DNA traces.
...
So, what is OONI? OONI stands for Open Observatory of Network Interference and is a sub-project of the Tor Project[^203].
...
The problem obviously is that your network providers will be able to see those tests and your attempts at connecting to various services if the network is monitored. The other issue is that there are solutions to prevent OONI from working properly[^346].
...
Usually, integrity checks[^347] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^348], more recently MD5[^349] but those present several weaknesses (CRC, MD5[^350]) that makes them unreliable for file integrity checks (which doesn't mean they're not still widely used in other contexts).

This is because they don't prevent Collision[^351] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having a different content.

For this reason, it's usually recommended to use SHA[^352] based hashes and the most commonly used is probably the SHA-2[^353] based SHA256 for verifying file integrity. SHA is much more resistant to collisions than CRC and MD5.
...
Integrity is one thing. Authenticity is another thing. This is a process where you can verify some information is authentic and from the expected source. This usually done by signing information (using GPG[^356] for instance) using public key cryptography[^357].
...
-   This practical cheat sheet from SANS: <https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf> (warning, many of those tools will be harder to use to Windows and you might consider using them from a Linux OS such as Tails, Whonix Workstation or a Linux distribution of your choice as explained later in this guide. There are also other guides out there[^358] that might be of use.
...
[^121]: Wikipedia, Cryptocurrency Tumbler <https://en.wikipedia.org/wiki/Cryptocurrency_tumbler>

[^122]: Wikipedia, Security Through Obscurity <https://en.wikipedia.org/wiki/Security_through_obscurity>

[^123]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^124]: SSRN, The Cryptocurrency Tumblers: Risks, Legality and Oversight <https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361>

[^125]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)>

[^126]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting>

[^127]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/>

[^128]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach>

[^129]: BBC, China social media: WeChat and the Surveillance State <https://www.bbc.com/news/blogs-china-blog-48552907>

[^130]: The Intercept, Revealed: Massive Chinese Police Database <https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/>
...
[^150]: EFF CoverMyAss, <https://coveryourtracks.eff.org/>
...
[^158]: MIT Technology Review, This is how we lost control of our faces, <https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/>

[^159]: Bellingcat, Shadow of a Doubt: Crowdsourcing Time Verification of the MH17 Missile Launch Photo <https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/>

[^160]: Brown Institute, Open Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^161]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^162]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^163]: APNews, Chinese \'gait recognition\' tech IDs people by how they walk <https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a>

[^164]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/>

[^165]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^166]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^167]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^168]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^169]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^170]: Anyvision <https://www.anyvision.co/>

[^171]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^172]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^173]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^174]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/>

[^175]: BBC, WhatsApp photo drug dealer caught by \'groundbreaking\' work <https://www.bbc.com/news/uk-wales-43711477>

[^176]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^177]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^178]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^179]: Wikipedia, Deepfake <https://en.wikipedia.org/wiki/Deepfake>

[^180]: Econotimes, Deepfake Voice Technology: The Good. The Bad. The Future <https://www.econotimes.com/Deepfake-Voice-Technology-The-Good-The-Bad-The-Future-1601278>

[^181]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events>

[^182]: Forbes, A Voice Deepfake Was Used To Scam A CEO Out Of \$243,000 <https://www.forbes.com/sites/jessedamiani/2019/09/03/a-voice-deepfake-was-used-to-scam-a-ceo-out-of-243000/>

[^183]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^184]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^185]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^186]: Wikipedia, Social Engineering <https://en.wikipedia.org/wiki/Social_engineering_(security)>

[^187]: Grayshirt, <https://www.grayshift.com/>

[^188]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf>

[^189]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^190]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^191]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^192]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^193]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^194]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^195]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^196]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^197]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^198]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^199]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^200]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone>

[^201]: Wikipedia, Rubber-hose Cryptanalysis https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis

[^202]: Defuse.ca, TrueCrypt\'s Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm>

[^203]: Wikipedia, OONI, <https://en.wikipedia.org/wiki/OONI>

[^204]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^205]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^206]: Usenix.org, Shedding too much Light on a Microcontroller's Firmware Protection <https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf>

[^207]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^208]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^209]: Coreboot, <https://www.coreboot.org/>

[^210]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid>

[^211]: XKCD, Password Strength <https://xkcd.com/936/>

[^212]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^213]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^214]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^215]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^216]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^217]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^218]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^219]: Wikipedia, APNS <https://en.wikipedia.org/wiki/Apple_Push_Notification_service>

[^220]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^221]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^222]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption>

[^223]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^224]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/>

[^225]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^226]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^227]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^228]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^229]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^230]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^231]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^232]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^233]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^234]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^235]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^236]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^237]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^238]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^239]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^240]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^241]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^242]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^243]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^244]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^245]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^246]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^247]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^248]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^249]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^250]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^251]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987>

[^252]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox>

[^253]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^254]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^255]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^256]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^257]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>
...
[^260]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^261]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^262]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^263]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^264]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^265]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^266]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^267]: hCaptcha <https://www.hcaptcha.com/>

[^268]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^269]: Nearcyan.com, You (probably) don't need ReCAPTCHA <https://nearcyan.com/you-probably-dont-need-recaptcha/>

[^270]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^271]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^272]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^273]: Tor Project Community, Cloudflare Captcha Monitoring <https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/>

[^274]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^275]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^276]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^277]: Developers Google Blog,
...
[^278]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^279]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^280]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^281]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^282]: DigitalInformationWorld, Facebook will now require you to Create a Video Selfie for Identity Verification <https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html>

[^283]: Vice.com, PornHub Announces \'Biometric Technology\' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id>

[^284]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify>

[^285]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^286]: Wikipedia, Sim Swapping <https://en.wikipedia.org/wiki/SIM_swap_scam>

[^287]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^288]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^289]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^290]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^291]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^292]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^293]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^294]: Jumio, ID verification features <https://www.jumio.com/features/>

[^295]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^296]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^297]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^298]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^299]: Privacytools.io, File-Sharing <https://privacytools.io/software/file-sharing/>

[^300]: Privacytools.io, Real-Time Communication <https://privacytools.io/software/real-time-communication/>

[^301]: Praxis Films, Open Letter from Laura Poitras <https://www.praxisfilms.org/open-letter-from-laura-poitras/>

[^302]: Wikipedia, SecureDrop <https://en.wikipedia.org/wiki/SecureDrop>

[^303]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^304]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^305]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^306]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^307]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^308]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^309]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^310]: ResearchGate, Live forensics method for acquisition on the Solid State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^311]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^312]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^313]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^314]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^315]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^316]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli>

[^317]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/>

[^318]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/>

[^319]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^320]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html>

[^321]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^322]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^323]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^324]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^325]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^326]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^327]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^328]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^329]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^330]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^331]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^332]: Tor Documentation, Snowflake, <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/>

[^333]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^334]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^335]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^336]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^337]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^338]: DBAN, <https://dban.org/>

[^339]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^340]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/>

[^341]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^342]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^343]: YouTube, Fingerprinting Paper - Forensic Education <https://www.youtube.com/watch?v=sO98kDLkh-M>

[^344]: Wikipedia, Touch DNA, <https://en.wikipedia.org/wiki/Touch_DNA>

[^345]: TheDNAGuide, DNA from Postage Stamps or Hair Samples? Yeeesssss\..... <https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps>

[^346]: GitHub, Mhinkie, OONI-Detection <https://github.com/mhinkie/ooni-detection>

[^347]: Wikipedia, File Verification <https://en.wikipedia.org/wiki/File_verification>

[^348]: Wikipedia, CRC <https://en.wikipedia.org/wiki/Cyclic_redundancy_check>

[^349]: Wikipedia, MD5 <https://en.wikipedia.org/wiki/MD5>

[^350]: Wikipedia, MD5 Security <https://en.wikipedia.org/wiki/MD5#Security>

[^351]: Wikipedia, Collision Resistance <https://en.wikipedia.org/wiki/Collision_resistance>

[^352]: Wikipedia, SHA <https://en.wikipedia.org/wiki/Secure_Hash_Algorithms>

[^353]: Wikipedia, SHA-2 <https://en.wikipedia.org/wiki/SHA-2>

[^354]: GnuPG Gpg4win Wiki, Check integrity of Gpg4win packages <https://wiki.gnupg.org/Gpg4win/CheckIntegrity>

[^355]: Medium.com, How to verify checksum on Mac <https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f>

[^356]: Wikipedia, GPG <https://en.wikipedia.org/wiki/GNU_Privacy_Guard>

[^357]: Wikipedia, Public-Key Cryptography <https://en.wikipedia.org/wiki/Public-key_cryptography>

[^358]: Lenny Zeltser, Analyzing Malicious Documents Cheat Sheet <https://zeltser.com/analyzing-malicious-documents/>
