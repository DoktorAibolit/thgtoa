...
This technology is also widely used in CAPTCHAS[^255] services to verify that you are "human" and can be used to fingerprint a user.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they're encrypted[^179]. These techniques are widely used by law enforcement all over the world and should be considered.
...
This is the hardest part of the whole guide. It's a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting more and more limited due to various KYC type regulations[^195].
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^224].

-   Windows Indexing keeping traces of the files present in your User folder by default[^225].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^226].
...
-   Gatekeeper[^227] and XProtect keeping track of your download history in a local database and file attributes.
...
Forensics could[^221]'[^224] use all those leaks[^231] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Veracrypt[^228] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^229] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^230] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^231] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^232] Operations[^233]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^202] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
In this route, we will make extensive use of the free Oracle Virtualbox[^241] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^242] have not be completely fixed to this date[^243] and if you're using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This route will use Virtualization and Whonix[^244] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later on you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^245] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
You will need something to store your data (logins/passwords, identities and TOTP[^249] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^250] authentication with the authenticator feature.
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^251], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
Captcha[^255] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^256] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^257]) and Cloudflare (hCaptcha[^258]). hCaptcha is used on 15% of the internet by their own metrics[^259].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^260]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^261].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^262].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^264].
...
Browser and Device[^265] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^266]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^267] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^268].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^269] and unfortunately sometimes required.
...
This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^270]. Unfortunately this method is now also sometimes being used on Facebook[^271] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^272].
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^273] instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^274]).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^266] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^202]**.**
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^275].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^276].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^277] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^280] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^281] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^282] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^283]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^284]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
This time you will need to submit an appeal using a form[^285], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using at least seven proxies[^286] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
-   Use an Android Emulator (Youwave[^287], Bluestacks[^288] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
So the fact is that it's very unlikely[^295]'[^296] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it's not completely impossible either[^297]'[^298]'[^299] if they're fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^300] for 11\$ and use it to erase any disk.
...
-   If your file system is APFS, you don't need to worry about Trim, it apparently happens asynchronously as the OS writes data[^306] according to their own documentation.
...
Unfortunately Apple has removed the secure erase options from the trash bin even for HDD drives[^307]. So you're left with using other tools:
...
The best tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^308] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It's however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^309] and available on TAILS by default[^310].
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^231]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^224] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally I would just pick Normal Deletion + Trim (Trim itself should be enough[^235]). Secure Deletion with Trim[^232] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^311] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^312] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^313] and Secure Communication Obfuscation[^314] through Steganography[^315] and/or Kleptography[^316] and has many names such as Koalang[^317] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> [^318]
...
As written earlier in this guide and as noted by Privacytools.io[^319], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   The Windows USN journal which stores plenty of information[^321].

-   The Windows System Resource Usage Monitor (SRUM)[^322].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^323].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^324]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
Some might be very efficient[^325] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It's pretty much the only way to be sure it's not only supported but actually works. Some utilities can tell you if it's supported or not like CrystalDiskInfo[^326] but won't actually check if it's working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives:]
...
According to Apple Documentation[^306], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
-   If you really have to take them with you, you could consider the use of a faraday cage[^327] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^328] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
[^175]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events>

[^176]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^177]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^178]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^179]: Grayshirt, <https://www.grayshift.com/>

[^180]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf>

[^181]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^182]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^183]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^184]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^185]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^186]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^187]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^188]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^189]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^190]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^191]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^192]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone>

[^193]: Wikipedia, Rubber-hose Cryptanalysis https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis

[^194]: Defuse.ca, TrueCrypt\'s Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm>

[^195]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^196]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^197]: Usenix.org, Shedding too much Light on a Microcontroller's Firmware Protection <https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf>

[^198]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^199]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^200]: Coreboot, <https://www.coreboot.org/>

[^201]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid>

[^202]: XKCD, Password Strength <https://xkcd.com/936/>

[^203]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^204]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^205]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^206]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^207]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^208]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^209]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^210]: Wikipedia, APNS <https://en.wikipedia.org/wiki/Apple_Push_Notification_service>

[^211]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^212]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^213]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption>

[^214]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^215]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/>

[^216]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^217]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^218]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^219]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^220]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^221]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^222]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^223]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^224]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^225]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^226]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^227]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^228]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^229]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^230]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^231]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^232]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^233]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^234]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^235]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^236]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^237]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^238]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^239]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^240]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^241]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^242]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987>

[^243]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox>

[^244]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^245]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^246]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^247]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^248]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>

[^249]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^250]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^251]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^252]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>
...
[^255]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^256]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^257]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^258]: hCaptcha <https://www.hcaptcha.com/>

[^259]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^260]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^261]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^262]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^263]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^264]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^265]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^266]: Developers Google Blog,
...
[^267]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^268]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^269]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^270]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^271]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^272]: Vice.com, PornHub Announces \'Biometric Technology\' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id>

[^273]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify>

[^274]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^275]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^276]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^277]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^278]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^279]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^280]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^281]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^282]: Jumio, ID verification features <https://www.jumio.com/features/>

[^283]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^284]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^285]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^286]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^287]: Youwave, <https://youwave.com/>

[^288]: Bluestacks, <https://www.bluestacks.com/>

[^289]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^290]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^291]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^292]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^293]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^294]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^295]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^296]: ResearchGate, Live forensics method for acquisition on the Solid State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^297]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^298]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^299]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^300]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^301]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^302]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli>

[^303]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/>

[^304]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/>

[^305]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^306]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html>

[^307]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^308]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^309]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^310]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^311]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^312]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^313]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^314]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^315]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^316]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^317]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^318]: Tor Documentation, Snowflake, <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/>

[^319]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^320]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^321]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^322]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^323]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^324]: DBAN, <https://dban.org/>

[^325]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^326]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/>

[^327]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^328]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>
