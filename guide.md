...
Version 0.7.9 (draft), March 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
-   Original: <https://anonymousplanet.github.io/>

-   Archive.today: <https://archive.fo/anonymousplanet.github.io>

-   Archive.today over Tor: <http://archivecaslytosk.onion/anonymousplanet.github.io> (**Online Version Only**)

Latest PDF versions (best format for the best readability) of this guide at:

-   (Light) <https://anonymousplanet.github.io/thgtoa/guide.pdf>

-   (Light Archive) <https://archive.fo/https://anonymousplanet.github.io/thgtoa/guide.pdf>

-   (Dark) <https://anonymousplanet.github.io/thgtoa/guide-dark.pdf>

-   (Dark Archive) <https://archive.fo/https://anonymousplanet.github.io/thgtoa/guide-dark.pdf>
...
This guide does mention and even recommend some commercial services in some sections (such as VPN and hosting providers) but is not endorsed or sponsored by any of them in any way. There are no referral links and no commercial ties with any of these providers.

Donations are welcome at <https://anonymousplanet.github.io/thgtoa/donations.html> or directly by sending:

**Monero to this address:**

![][21]

"monero:4549BGJrEPBfpiPRL9CVGzGMgJnC1Dzf8EXLVfY8Ukrnj7LzkTV611dGf9tuQHiSQjbixsNWiffNiV5fPB3LkyF7UXi3vwQ" **(please do verify the checksum and gpg signature of this file for authenticity, this is explained in the README of the repository if you do not know how to do that)**.

**Bitcoin to this address:**

"bc1qekuxlepa52j40ntwj7rrga823d9v0rd4zzwtwz" **(please do verify the checksum and gpg signature of this file for authenticity, this is explained in the README of the repository if you do not know how to do that)**.

![][22]

Follow me on Twitter <https://twitter.com/AnonyPla> (cannot guarantee this account will stay up for long tho)
...
-   [Appendix Q: How to bypass (some) local restrictions on supervised computers]
    -   [Portable Apps:]
...
Making a social media account with a pseudonym or artist/brand name is easy. And it is enough is most use cases to protect your identity as the next George Orwell. There are plenty of people using pseudonyms all over Facebook/Instagram/Twitter/LinkedIn/TikTok/Snapchat/Reddit/... But the vast majority of those are anything but anonymous and can easily be traced to their real identity by your local police officers, random people within the OSINT[^2] (Open-Source Intelligence) community and trolls[^3] on 4chan[^4].
...
Apple devices are IMHO among the best choices for good Privacy but not necessarily for Anonymity. They will protect people from others knowing what you are doing but not protect you from others (Apple themselves for instance) knowing who you are.

Will this guide help you protect yourself from the NSA, the FSB, Mark Zuckerberg, or the Mossad if they are out to find you? Probably not ... Mossad will be doing "Mossad things" [^6] and will probably find you no matter how hard you try to hide[^7].

You must consider your threat model[^8] before going further.

![][23]

(Illustration by xkcd.com, licensed under CC BY-NC 2.5)

Will this guide help you protect your privacy from OSINT researchers like Bellingcat[^9] , Doxing[^10] trolls on 4chan[^11] and others that have no access to the NSA toolbox? More likely. Tho I would not be so sure about 4chan.
...
-   **Be a permanent Adult resident in Germany where the courts have upheld up the legality of not using real names on online platforms (§13 VI of the German Telemedia Act of 2007**[^18]**'**[^19]**). Alternatively, be an Adult resident of any other country where you can validate and verify the legality of this guide yourself.**
...
-   Don't be evil (for real this time)[^20].
...
For those reasons, we will need to not use that origin IP (the one tied to your identification) or hide it as much as we can through a combination of various means:

-   Using a public WIFI service (free).

-   Using the Tor Anonymity Network[^23] (free).

-   Using a VPN[^24] service anonymously (anonymously paid).

All those will be explained later in this guide.

## Your DNS requests:

DNS stands for "Domain Name System"[^25] and is a service used by your browser (and other apps) to find the IP addresses of a service. It is pretty much a huge "contact list" (phone book for older people) that works like asking it a name and it returns the number to call. Except it returns an IP instead.
...
Here is a video explaining DNS visually if you are already lost: <https://www.youtube.com/watch?v=vrxwXXytEuI>

Usually, the DNS service is provided by your ISP and automatically configured by the network you are connecting to. This DNS service could also be subject to data retention regulations or will just keep logs for other reasons (data collection for advertising purposes for instance). Therefore, this ISP will be capable of telling everything you did online just by looking at those logs which can in turn be provided to an adversary. Conveniently this also the easiest way for many adversaries to apply censoring or parental control by using DNS blocking[^26]. The provided DNS servers will give you a different address (than their real one) for some websites (like redirecting thepiratebay to some government website). Such blocking is widely applied worldwide for certain sites[^27].
...
Because it is not encrypted, your ISP and/or any other adversary could still intercept (using a Man-in-the-middle attack[^58]) your request will know and possibly log what your IP was looking for. The same ISP can also tamper with the DNS responses even if you are using a private DNS. Rendering the use of a private DNS service useless.

As a bonus, many devices and apps will use hardcoded DNS servers bypassing any system setting you could set. This is for example the case with most (70%) Smart TVs and a large part (46%) of Game Consoles[^28]. For these devices, you will have to force them[^29] to stop using their hardcoded DNS service which could make them stop working properly.

A solution to this is to use encrypted DNS using DoH (DNS over HTTPS[^30]), DoT (DNS over TLS[^31]) with a private DNS server (this can be self-hosted locally with a solution like pi-hole[^32], remotely hosted with a solution like nextdns.io or using the solutions provider by your VPN provider or the Tor network). This should prevent your ISP or some middle-man from snooping on your requests ... except it might not.

Unfortunately, the TLS protocol used in most HTTPS connections in most Browsers (Chrome/Brave among them) will leak the DNS again through SNI[^33] handshakes (this can be checked here at Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> ). **As of the writing of this guide**, **only Firefox based browsers supports eSNI (encrypted SNI**[^34] **soon to be renamed ECH**[^35] **and soon to be supported by Firefox too**[^36]**) which will encrypt everything end to end (in addition to using a secure private DNS over TLS/HTTPS) and will allow you to hide your DNS requests from a third party**[^37]**.** And this option is not enabled by default either so you will have to enable it yourself:

In addition to limited browser support, only Web Services and CDNs[^38] behind Cloudflare CDN support eSNI at this stage (and will support ECH in the future[^39]). This means that eSNI (or soon ECH) is not supported (as of January 2021) by many mainstream platforms such as:
...
Some countries like Russia[^40] and China[^41] will block eSNI handshakes at network level to allow snooping and prevent bypassing censorship. Meaning you will not be able to establish an HTTPS connection with a service if you do not allow them to see what it was.

Finally, even if you use a custom encrypted DNS server (DoS or DoT) with eSNI support, it might still not be enough as traffic analysis studies[^42] have shown it is still possible to reliably fingerprint and block unwanted requests. Only DNS over Tor was able to demonstrate efficient DNS Privacy in recent studies but even that can still be defeated by other means (see [Your "Anonymized" Tor/VPN traffic:][25]).

One could also decide to use a Tor Hidden DNS Service or ODoH (Oblivious DNS over HTTPS[^43]) to further increase privacy/anonymity but unfortunately, as far as I know, these services are only provided by Cloudflare as of this writing (<https://blog.cloudflare.com/welcome-hidden-resolver/>, https://blog.cloudflare.com/oblivious-dns/). I **[personally]{.ul}** think these are viable and reasonably secure technical options but there is also a moral choice if you want to use them (despite the risk posed by some researchers[^44]).

Lastly, there is also this new option called DoHoT which stands for DNS over HTTPS over Tor which could also further increase your privacy/anonymity and which you could consider if you are more skilled with Linux. See <https://github.com/alecmuffett/dohot>. This guide will not help you with this one at this stage but it might be coming soon.
...
-   The smartphone OS makers (Google/Apple for Android/IOs) also keep logs of IMEI/IMSI identifications tied to Google/Apple accounts and which user has been using them. They too can trace back the history of the phone and to which accounts it was tied in the past[^53].

-   Government agencies around the world interested in your phone number can and do use[^54] special devices called "IMSI catchers"[^55] like the Stingray[^56] or more recently the Nyxcell[^57]. These devices can impersonate (to spoof) a cell phone Antenna and force a specific IMSI (your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^58] (Man-In-The-Middle Attacks) that will allow them to:
...
Your home router/WIFI access point keeps logs of devices that registered on the Wi-Fi and these can be accessed too to find out who has been using your WIFI. Sometimes this can be done remotely (and silently) by the ISP depending if that router/Wi-Fi access point is being "managed" remotely by the ISP (which is often the case when they provide the router to their customers).
...
Your Bluetooth MAC is like a MAC address except it is for Bluetooth. Again, it can be used to track you as manufacturers and operating system makers keep logs of such information. It could be tied to a sale place/time/date or accounts and then could be used to track you with such information, the shop billing information, the CCTV, or the mobile antenna logs in correlation.
...
-   IOS/Apple:
    ...
    -   Apple does claim[^62] that they anonymize this data using differential privacy[^63] but you will have to trust them on that.
...
But that is not all those WiFis access points can do. Recently developed techs could even allow someone to track your movements accurately just based on radio interferences. What this means is that it is possible to track your movement inside a room/building based on the radio signals passing through. This might seem like a tinfoil hat conspiracy theory claim but here are the references[^66] with demonstrations showing this tech in action: <http://rfpose.csail.mit.edu/> and the video here: <https://www.youtube.com/watch?v=HgDdaMy8KNE>
...
Your metadata is all the information about your activities without the actual content of those activities. For instance, it is like knowing you had a call from an oncologist before then calling your family and friends successively. You do not know what was said during the conversation but you can guess what it was just from the "metadata"[^67].
...
All this data is very likely being transmitted, processed, and stored indefinitely (unencrypted[^81]) by various third parties[^82].

But that is not all, this section is not called "Smartphones" but "Smart devices" because it's not only your smartphone spying on you. It is also every other smart device you could have.
...
You have seen this in action/spy/Sci-Fi movies and shows, the protagonists always remove the battery of their phones to make sure it cannot be used. Well, this is now true at least for some devices:

-   iPhones and iPads (IOS 13 and above)[^89]'[^90].

-   Samsung Phones (Android 10 and above)[^91].

-   MacBooks (MacOS 10.15 and above)[^92].

Such devices will continue to broadcast identity information to nearby devices even when offline using Bluetooth Low-Energy[^93]. They do not have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^94].
...
RFID stands for Radio-frequency identification[^95], it is the technology used for instance for contactless payments and various identification systems. Of course, your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^96]. As with everything else, such capabilities can be used for tracking by various actors.
...
You should always use extreme caution. To mitigate these attacks, this guide will later recommend the use of virtualization to mitigate leaking any information even in case of opening such a malicious file.

If you want to learn how to detect such malware, see [Appendix T: Checking files for malware]
...
Such watermarks can be inserted by various products[^102]'[^103]'[^104]'[^105] using Steganography[^106] and can resist compression[^107] and re-encoding[^108]'[^109].

These watermarks are not easily detectable and could allow identification of the source despite all efforts.

In addition to watermarks, the camera used for filming (and therefore the device used for filming) a video can also be identified using various techniques such as lens identification[^110] which could lead to de-anonymization.
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted traffic over the years[^111]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   Correlation Fingerprinting Attack: As illustrated (simplified) below, this attack will fingerprint[^112] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It can do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][31]

-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^113] a bomb threat hoax at Harvard University.
...
Be aware again that this might not be enough against a motivated global adversary[^114] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor can already be considered a suspicious activity[^115] and its use could be considered malicious by some[^116].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^117]. Most crypto currencies can be tracked accurately through various methods[^118].

Remember what they say on their own page: <https://bitcoin.org/en/you-need-to-know> and <https://bitcoin.org/en/protect-your-privacy>:

"Bitcoin is not anonymous "

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You will have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^119] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy/anonymity in mind like Monero but even those have some and warnings to consider[^120]'[^121].

Even if you use Mixers or Tumblers[^122] (services that specialize in "anonymizing" crypto currencies by "mixing them"), keep in mind this is only obfuscation[^123] and not actual anonymity[^124]. Not only are they only obfuscation but they could also put you in trouble as you might end up exchanging your crypto against "dirty" crypto that was used in various questionable contexts[^125].

This does not mean you cannot use Bitcoin anonymously at all. You can actually use Bitcoin anonymously as long as you do not convert it to actual currency and use a Bitcoin wallet from a safe anonymous network. Meaning you should avoid KYC regulations by various exchanges and avoid using the Bitcoin network from any known IP address. This also means you will not be able to convert these Bitcoin from/to real currency easily and without involving some risks.
...
So, you are using Tor Browser or Brave Browser over Tor. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^126] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^127] case in 2013 where the FBI inserted malware[^128] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^129] hack that breached several US government institutions by inserting malware into an official software update server.

In some countries, Malware is just mandatory and/or distributed by the state itself. This is the case for instance in China with WeChat[^130] which can then be used in combination with other data for state surveillance[^131].
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^132] without being able to access identifying information or compromise your system.

## Your CPU:

All modern CPUs[^133] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^134] and the AMD Platform Security Processor[^135].
...
These have already been affected by several security vulnerabilities in the past[^136] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^137].

There are some not so easy ways[^138] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.

If you are feeling a bit more adventurous, you could install your own BIOS using Libreboot[^139] or Coreboot[^210] if your laptop supports it.
...
-   If you are using Linux you can check the vulnerability status of your CPU to Spectre/Meltdown attacks by using <https://github.com/speed47/spectre-meltdown-checker> which is available as a package for most Linux distros including Whonix.

-   If you are using Windows you can check the vulnerability status of your CPU using inSpectre <https://www.grc.com/inspectre.htm>

Some of these can be avoided using Virtualization Software settings that can mitigate such exploits. See this guide for more information <https://www.whonix.org/wiki/Spectre_Meltdown> (warning: these can severely impact the performance of your VMs).
...
These have been used since at least since 2008 using an attack called "Jasager"[^140] and can be done by anyone using self-built tools or using commercially available devices such as Wi-Fi Pineapple[^141].
...
These devices can fit in a small bag and can take over the Wi-Fi environment of any place within their range. For instance, a Bar/Restaurant/Café/Hotel Lobby. These devices can force Wi-Fi clients to disconnect from their current Wi-Fi (using de-authentication, disassociation attacks[^142]) while spoofing the normal Wi-Fi networks at the same location. They will continue to perform this attack until your computer or yourself decides to try to connect to the rogue AP.

These devices can then mimic a captive portal[^143] with the exact same layout as the Wi-Fi you are trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you open access internet that they will themselves get from the same place.
...
There are readily available commercial and cheap "badUSB" [^144]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^145] in 2005.

While you could inspect an USB key physically, scan it with various utilities, check the various components to see if they are genuine, you will most likely never be able to discover complex malware embedded in genuine parts of a genuine USB key by a skilled adversary without advanced forensics equipment[^146].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^147], Cellebrite Cloud[^148]) that will help an adversary analyze your cloud data with ease.
...
The digital fingerprint is the way you write[^149], the way you behave[^150]. The way you click. The way you browse. The fonts you use on your browser[^151]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
This technology is also widely used in CAPTCHAS[^265] services to verify that you are "human" and can be used to fingerprint a user.

Analysis algorithms could then be used to match these patterns with other users and match you to a different known user. It is unclear if such data is used or not by Governments and Law Enforcements agencies but it might be in the future. And while this might only be used for advertising/marketing purposes now. It could and probably will be used for investigations in the short or mid-term future.
...
There are also a few cases involving OSINT at Bellingcat[^153].Have a look at their very informative toolkit here: <https://bit.ly/bcattools>
...
Your Browser and Device Fingerprints[^277] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance, websites will be able to provide a "mobile experience" if you are using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^154] based browsers (such as Chrome) or Firefox[^155] unless taking special measures.
...
This guide will mitigate these issues by mitigating, obfuscating, and randomizing many of those fingerprinting identifiers by using Virtualization and using by fingerprinting resistant Browsers.
...
Even if you are not looking at the camera, they can still figure out who you are[^162], make out your emotions[^163], analyze your gait[^164] and probably guess your political affiliation[^165].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^166]'[^167]'[^168]'[^169]'[^170].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^171]) in their CCTV networks to look for "persons of interest"[^172]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^173] including possibly identifying ethnic minorities[^174]. A simple face recognition error by some algorithm can ruin your life[^175].
...
Same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^176].

We can safely imagine a near future where you will not be able to create accounts or sign-in anywhere without providing unique biometrics (A good time to re-watch Gattaca[^177], Person of Interest[^178] and Minority Report[^179]). And you can safely imagine how useful these large biometrics databases could be to some interested third parties.

In addition, all this information can also be used against you (if you are already de-anonymized) using deepfake[^180] by crafting false information (Pictures, Videos, Voice Recordings[^181]...) and have already been used for such purposes[^182]'[^183]. There are even commercial services for this readily available such as <https://www.respeecher.com/> and <https://www.descript.com/overdub>.

See this demo: <https://www.youtube.com/watch?v=t5yw5cR79VA>

At this time, there are a few steps[^184] you can use to mitigate (and only mitigate) face recognition when conducting sensitive activities where CCTV might be present:

-   Wear a facemask as they have been proven to defeat some face recognition technologies[^185].
...
Phishing[^186] is a social engineering[^187] type of attack where an adversary could try to extract information from you by pretending to be something/someone else.

A typical case is an adversary using a man-in-the-middle[^58] attack or a falsified e-mail/call to ask your credential for a service. This can be your e-mail or your financial services for example.
...
These have been used countless times and the usual one is called the "419 scam" (see <https://en.wikipedia.org/wiki/Advance-fee_scam>).
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they are encrypted[^188]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And finally, here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^189]
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^190] or a national security letter[^191]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^192].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^193].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^194].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^195].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^196].

Some providers have implemented the use of a Warrant Canary[^197] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Finally, it is now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^198] and Omnisec story[^199]).
...
There are many advanced techniques that can be used by skilled adversaries[^200] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^201] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw>
...
-   GitHub Transparency Report <https://github.blog/2021-02-25-2020-transparency-report/>
...
You might also enjoy looking at this service <https://tosdr.org/> (Terms of Services, Didn't Read) that will give you a good overview of the various ToS of many services.

Have a look at <https://www.eff.org/issues/privacy> for more resources.

Have a look at <https://en.wikipedia.org/wiki/List_of_government_mass_surveillance_projects> to have an overview of all known mass-surveillance projects.
...
-   You are a l33T hacker, "there is no spoon", "the cake is a lie", you've been using "doas" for years and "all your base are belong to us"
    ...
    -   **This guide is not really meant for you and will not help you with OpenBSD ;-)**
...
-   If you absolutely want system wide plausible deniability[^221]'[^202] despite the risks[^203]'[^223].

    -   **You could go with the TAILS Route including the persistent plausible deniability section.**

    -   **You could go with the Whonix Route (on Windows Host OS only).**
...
-   Check some data yourself here on the Tor Project OONI[^204] (Open Observatory of Network Interference) website: <https://explorer.ooni.org/>
...
**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^212]**.**
...
You should test that the phone is in working order before going to the next step. But I will repeat myself and state again that it is really important that you leave your smartphone at home when going (or turn it off before leaving if you must keep it) and that you test the phone at a random location that cannot be tracked back to you (and again, do not do that in front of a CCTV, avoid cameras, be aware of your surroundings). No need for WIFI at this place either.
...
This is the hardest part of the whole guide. It is a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting increasingly limited due to various KYC type regulations[^205].

So here is a list of places where you can still get them now: <https://prepaid-data-sim-card.fandom.com/wiki/Registration_Policies_Per_Country>
...
Personally, I would recommend GiffGaff in the UK as they are "affordable", do not require identification for activation and top-pup and will even allow you to change your number up to 2 times from their website. One GiffGaff prepaid SIM card will therefore grant you 3 numbers to use for your needs.
...
Some might be very efficient[^206] but many are gimmicky gadgets that offer no real protection[^207].

### Find some safe places with decent public WIFI:

You need to find safe places where you will be able to do your sensitive activities using some publicly accessible WIFI (without any account/ID registration, avoid CCTVs).

This can be anywhere that will not be tied to you directly (your home/work) and where you can use the WIFI for a while without being bothered. But also, a place where you can do this without being "noticed" by anyone.
...
-   You will need to buy a coffee to get the WIFI access code in most. If you pay this coffee with an electronic method, they will be able to tie your WIFI access with your identity.
...
TAILS[^208] stands for **The Amnesic Incognito Live System**. It is a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
-   Tails will not protect you more from the 5\$ wrench[^7].

-   Tor in itself might not be enough to protect you from an adversary with enough resources as explained earlier.

**Important Note: If your laptop is monitored/supervised and some local restrictions are in place, please read [Appendix Q: How to bypass (some) local restrictions on supervised computers].**
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^209]**.**
...
-   Download the latest HiddenVM release from <https://github.com/aforensics/HiddenVM/releases>

-   Download the latest Whonix XFCE release from <https://www.whonix.org/wiki/VirtualBox/XFCE>
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm>) or System76 laptops (<https://system76.com/>) while using Coreboot[^210] (where Intel IME is disabled from factory).
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. However, if you are going for the Qubes Route consider Intel as they do not support AMD with their anti-evil-maid system[^211].
...
-   Enable BIOS/UEFI password and use a long passphrase[^212] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^213]. Keep it on if you intend to use Linux/Windows.
...
So, what is Secure Boot[^214]? In short, it is a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically, when the Operating Systems (or the Bootloader[^215]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
**So, it can be useful as an added measure against some adversaries but not all. Secure Boot in itself is not encrypting your hard drive. It is an added layer but that's it.**
...
This route will make extensive use of Virtual Machines[^217], they will require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for anonymity depending on your threat model. This is because using Windows will allow us to use Plausible Deniability[^221] easily at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^222] but is the only (convenient) option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.

So, what is Plausible Deniability? It is the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^223].

A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^224]'[^225] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will come into play.
...
Cold Boot attacks[^228] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you are actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^229], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability, there have been some forensics studies[^230] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^231] and by the maintainer of Veracrypt[^232] so I would not worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^234].

-   Windows Indexing keeping traces of the files present in your User folder by default[^235].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^236].
...
-   Gatekeeper[^237] and XProtect keeping track of your download history in a local database and file attributes.
...
Forensics could[^231]'[^234] use all those leaks (see [Local Data Leaks and Forensics:]) to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
You should always remember that despite the reputation, Linux mainstream distributions (Ubuntu for instance) are not necessarily better at security than other systems such as MacOS and Windows. See this reference to understand why <https://madaidans-insecurities.github.io/linux.html>.
...
-   If you are not sure, just make sure you did not enable any telemetry and follow this tutorial if needed <https://vitux.com/how-to-force-ubuntu-to-stop-collecting-your-data-from-your-pc/>
...
Please refer to this guide <https://madaidans-insecurities.github.io/guides/linux-hardening.html>
...
Finally, due to the issues mentioned earlier about Big Sur and telemetry, I do not recommend using Big Sur for now and recommend instead staying on MacOS Catalina until the situation is "clarified".
...
Veracrypt[^238] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^239] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^240] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which does not clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^241] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you will have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^242] Operations[^243]. This will severely degrade the performance/health of your SSD drive over time.

**As you can see, Route C only offers two privacy advantages over the others and it will only be of use against a soft lawful adversary. Remember <https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis>.**
...
-   Use a strong passphrase[^212] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^250] or not rescue disk, well that is up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you will still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^251] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^252] have not be completely fixed to this date[^253] and if you are using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This is my recommended and preferred method.
...
This is because many services will just outright ban, hinder, or block Tor (see <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor>).

#### Tor only:

This other illustration shows the other possibility without a VPN. It is very similar but has one less defense layer. If the Tor network is compromised by a Global/State actor, then you will be likely de-anonymized faster.
...
-   Your VPN provider is just another ISP that will then know your origin IP and will be able to de-anonymize you if required. We do not trust them. I prefer a situation where your VPN provider does not know who you are.
...
So, if you must use a VPN to access Tor, I would then recommend using another scheme instead:
...
This route will use Virtualization and Whonix[^254] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Finally, also follow this recommendation to desync the clock you are your VM compared to your host OS <https://www.whonix.org/wiki/Network_Time_Synchronization#Spoof_the_Initial_Virtual_Hardware_Clock_Offset>
...
Skip this step if you cannot use Tor because it is too dangerous/suspicious.
...
Remember at this stage that if you are having issues connecting to Tor due to censorship or blocking, you should consider connecting using Bridges as explained in this tutorial <https://www.whonix.org/wiki/Bridges>.
...
If you cannot use Tor, you can use a Custom VM of your choice that will ideally use an anonymous VPN if possible, to then connect to the Tor network.
...
-   Keep the disk size at 50GB (this is a maximum, it will not reach that much)
...
Basically, follow the tutorial here: <https://www.android-x86.org/documentation/virtualbox.html>

-   Download the appropriate ISO file (for instance x86_64_k49 for running from an Intel or AMD 64 Bit processor with Linux Kernel 4.9).
...
You will need something to store your data (logins/passwords, identities and TOTP[^259] information).

For this purpose, I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^260] authentication with the authenticator feature.
...
Qubes OS is different from other Linux distributions because it will make extensive use of Virtualization and Compartmentalization so that any app will run in a different VM (qube). As a bonus, Qubes OS integrates Whonix by default and allows for increased privacy and anonymity. It is highly recommended that you document yourself over Qubes OS principles prior to going this route. Here are some recommended resources:
...
Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^261], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
This is my preferred and recommend method.
...
If they somehow compromise your Qube App VM (with a malware/exploit), they will be faced with an internal IP of Qube OS and should not be able to figure out the Public Wi-Fi IP.
...
This other illustration shows the other possibility without a cash (preferred)/Monero paid VPN. It is very similar but has one less defense layer. If the Tor network is compromised by a Global/State actor, then you will be likely de-anonymized faster.
...
-   Your VPN provider is just another ISP that will then know your origin IP and will be able to de-anonymize you if required. We do not trust them.
...
-   Install Qubes OS according to the installation guide:
    ...
    -   **If you want to use Tor or VPN over Tor: Check the** "**Enabling system and template updates over the Tor anonymity network using Whonix" during the last step. This will force all Qubes OS updates to go through Tor. While this will significantly reduce your update speed, it will increase your anonymity from the start.** (If you are having issues connecting to Tor due to censorship or blocking, consider using Tor Bridges as recommended earlier. Just follow the tutorial provided here: <https://www.whonix.org/wiki/Bridges>)
...
-   Test your Connectivity:
    ...
    -   If you are going for VPN over Tor, Test the VM connectivity to Tor by launching a Browser within the ProxyVM and going to <https://check.torproject.org> (It should say you are connected to Tor)
...
> (These will block outbound traffic when the VPN is down, it is a kill switch, more information here <https://linuxconfig.org/how-to-create-a-vpn-killswitch-using-iptables-on-linux> )
...
Now you will have to follow the instructions from here: <https://github.com/anbox/anbox-modules>
...
That's it, you should now have an Android Qube over Tor (or anything else) capable of running pretty much any App you can sideload with ADB. This is IMHO the easiest way to get Android emulation on Qubes OS at the moment.

### KeePassXC:

You will need something to store your data (logins/passwords, identities and TOTP[^263] information).

For this purpose, I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^264] authentication with the authenticator feature.
...
Captcha[^265] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^266] puzzles you need to complete before accessing a form/website. You will mostly encounter those provided by Google (reCaptcha service[^267]) and Cloudflare (hCaptcha[^268]). hCaptcha is used on 15% of the internet by their own metrics[^269].

They are designed to separate bots from humans but are also used to deter anonymous and private users.

If you frequently use VPNs or Tor, you will quickly encounter many captchas everywhere[^270]. Quite often when using Tor, even if you succeed in solving all the puzzles, you will still be denied after solving the puzzles.

See <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor>

While most people think those puzzles are only about solving a little puzzle, it is important to understand that it is much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you are human[^271]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^272].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it is "human".

-   They track your behavior before/during/after the tests to ensure you are "human"[^273].
...
You will often experience several in a row (sometimes endless[^274]) and sometimes very difficult ones involving reading undecipherable characters or identifying various objects on endless pictures set. You will also have more captchas if you use ad blocking system or if your account was flagged for any reason for using VPNs or Tor previously.

You will also have (in my experience) more Captchas (reCaptcha) in Google if you do not use Chrome. But this can be mitigated by using Chromium based browsers such as Brave. There is also a Browser extension called Buster that could help you those <https://github.com/dessant/buster>.

As for Cloudflare (hCaptcha), you could also use their Accessibility solution here (<https://www.hcaptcha.com/accessibility>) which would allow you to sign-up (with your anonymous identity created later) and set a cookie within your Browser that would allow you to bypass their captchas. Another solution to mitigate hCaptcha would be to use their own solution called "Privacy Pass"[^275] <https://privacypass.github.io/> in the form of a Browser extension you could install in your VM Browser.
...
Most platforms (including the privacy-oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^276].
...
Browser and Device[^277] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^278]) will check your browser for various capabilities and settings and block Browsers they do not like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^279] on Netflix as they cover this topic much better than anyone else IMHO.
...
While this might seem innocent, this is obviously an ID verification and de-anonymization method. This is just indirectly relying on third party financial KYC[^119] regulations.

This is for instance now the case on YouTube for some European Users[^280] but also used by services like Amazon that requires a valid payment method for creating an account.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^281] and unfortunately sometimes required.
...
This guide will not cover this one (yet) as it is mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^282]. Unfortunately, this method is now also sometimes being used on Facebook[^283] and Instagram as part of their verification methods (tho I did not face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^284].
...
Cons: Usually that verdict is "final" and you will probably be banned without any appeal possibility if you are not good.
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^285] instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is the real term for this[^286]).
...
-   Identities that are "EU residents" with an "EU IP" (VPN/Tor) will benefit from GDPR protections on many platforms. Others will not. GDPR is your friend in most cases and you should take this into account.
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will experience less issues with captchas and online platforms[^278] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Every identity you should have a matching profile picture associated to it. For this purpose, I recommend you just go to <https://thispersondoesnotexist.com/> and generate a computer-generated profile picture. You can also generate such pictures yourself from your computer if you prefer by using the open-source StyleGan project here <https://github.com/NVlabs/stylegan2>. Just refresh the page until you find a picture that matches your identity in all aspects (age, sex, and ethnicity) and save that picture. It would be even better to have several pictures associated to that identity but I do not have an "easy way" of doing that yet.
    ...
    -   **Bonus**, you could also make it more real by using this service (with an anonymous identity) <https://www.myheritage.com/deep-nostalgia> to make a picture more lifelike. Here is an example:
...
You could also achieve the same result without using MyHeritage and by doing it yourself using for example <https://github.com/AliaksandrSiarohin/first-order-model> but this will require more manual operations (**and requires an NVIDIA GPU**).
...
-   Always use TOTP 2FA (not SMS to prevent Sim Swapping attacks[^287] and to keep your identity working when your pre-paid card expires) using KeePassXC when available to secure your logins to various platforms.

-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^212]**.**
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^288].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^289].
...
**Please note that this is restricting Onion Routing could limit your Anonymity if you are too restrictive. You can see a visualized list of available Exit Nodes here: <https://www.bigdatacloud.com/insights/tor-exit-nodes>**
...
On my side, I strongly condemn this type of real-name policy. See for instance this Wikipedia article giving some examples: <https://en.wikipedia.org/wiki/Facebook_real-name_policy_controversy>
...
-   South Korea (see <https://en.wikipedia.org/wiki/Real-name_system#South_Korea>)

-   If you know any other, please let me know with references in the GitHub issues.

Some platforms are by-passing this requirement all-together by requiring a valid payment method instead (see [Financial transactions:]). While this does not directly require a real-name through their ToS, this has the same results as they usually only accept mainstream (not Monero/Cash) payment methods (such as Visa/MasterCard/Maestro or PayPal) which do require a real-name legally as part of their KYC[^119] regulations. The result is the same and arguably even better than a simple real-name policy you could ignore in some countries such as Germany.
...
-   Is this against their ToS? No but yes <https://www.amazon.com/gp/help/customer/display.html?nodeId=202140280>
...
-   Is this against their ToS? Yes <https://www.apple.com/legal/internet-services/icloud/en/terms.html>
...
-   Is this against their ToS? No <https://briarproject.org/privacy-policy/>

-   Will they require a phone number? No, they do not even require an e-mail

-   Can you create accounts through Tor? Yes

**Remember this app requires an Android emulator. There is no official desktop client AFAIK.**
...
-   Is this against their ToS? No <https://element.io/terms-of-service>
...
-   Can you create accounts through Tor? Yes, but it is very difficult and their onion address[^290] will not help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in."
...
-   Is this against their ToS? No <https://about.gitlab.com/handbook/legal/subscription-agreement/>
...
If your account gets suspended [^293] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^294] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
It is also possible that Google will require an ID check through indirect financial KYC or ID picture check if you attempt to access/publish mature content on their platform[^295].

#### HackerNews:

-   Is this against their ToS? No <https://www.ycombinator.com/legal/#tou>
...
-   Is this against their ToS? **Maybe?** I am not sure <https://help.instagram.com/581066165581870?ref=dp>
...
-   Is this against their ToS? No <https://jami.net/privacy-policy/>
...
-   Is this against their ToS? No <https://www.ivpn.net/tos/>
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^296] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
-   Is this against their ToS? No unless it is about crypto <https://policy.medium.com/medium-terms-of-service-9db0094a1e0f>
...
-   Is this against their ToS? No <https://mullvad.net/en/help/terms-service/>
...
-   Is this against their ToS? No <https://njal.la/tos/>
...
They are recommended by privacytools.io[^297]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^298]. If their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
-   Is this against their ToS? No <https://protonvpn.com/terms-and-conditions>
...
-   Is this against their ToS? Yes <https://slashdotmedia.com/terms-of-use/>
...
This time you will need to submit an appeal using a form[^299], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it is possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it is you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
-   Is this against their ToS? No <https://www.twitch.tv/p/en/legal/terms-of-service/>
...
-   Is this against their ToS? **Yes** <https://www.whatsapp.com/legal/updates/terms-of-service-eea>
...
You are going to have to find a different way to post there using at least seven proxies[^300] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
For this purpose, I will not recommend classic messengers like Signal/Telegram as those require a phone number.

Instead, you should use better options (as recommend by privacytools.io[^301]'[^302]) such as:

-   Options requiring an Android emulator:

    -   Briar (<https://briarproject.org/>)

-   Options running on Host OSes (and integrated into Whonix and TAILS by default)
...
**Note that all those options should ideally be used at least over Tor for safety (from TAILS or a guest OS running behind the Whonix Gateway such as the Whonix Workstation).**

All these options will not require any identifiable information.

### Communicating sensitive information to various known organizations:

You might be interested in communicating information to some organization such as the press anonymously.

If you must do so, you should take some steps because you cannot really trust any organization to protect your anonymity[^303]:
...
For this, I strongly recommend the use of SecureDrop[^304] (<https://securedrop.org/>) which is an open-source project from the Freedom of the Press foundation.

Ideally you should use SecureDrop over Tor and you will find a curated list of those here <https://github.com/alecmuffett/real-world-onion-sites>
...
-   Check regularly the eventual appearance of compromise of any of your identities using <https://haveibeenpwned.com/> (obviously from a safe environment).
...
The process is fairly simple and all you will need is to follow Veracrypt tutorial here: <https://www.veracrypt.fr/en/Beginner%27s%20Tutorial.html>
...
**You should however consider formatting this drive as exFAT to that it is also usable by other OSes conveniently (Windows/Linux) without added software using this guide: <https://support.apple.com/en-ie/guide/disk-utility/dskutl1010/mac>**
...
**Consider tho that only one of those accepting Monero is recommended by Privacytools.io (<https://privacytools.io/providers/hosting/>) and it is Njalla <https://njal.la/>**
...
So, the fact is that it is very unlikely[^311]'[^312] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it is not completely impossible either[^313]'[^314]'[^315] if they are fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^316] for 11\$ and use it to erase any disk.
...
-   If your file system is APFS, you do not need to worry about Trim, it apparently happens asynchronously as the OS writes data[^322] according to their own documentation.
...
Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^323]. So, you are left with using other tools:
...
On Windows, MacOS and Linux I would recommend ExifTool (<https://exiftool.org/>) and/or ExifCleaner (<https://exifcleaner.com/>) that allows viewing and/or removing those properties.

**ExifTool is natively available on TAILS and Whonix Workstation.**

##### ExifCleaner:

Just install it from <https://exifcleaner.com/>, run and drag and drop the files into the GUI.
...
Consider the use of Fawkes <https://sandlab.cs.uchicago.edu/fawkes/> (<https://github.com/Shawn-Shan/fawkes>) to cloak the images from picture recognition tech on various platforms.
...
Alternatively, on Windows, MacOS and Linux I would recommend ExifTool (<https://exiftool.org/>) and/or ExifCleaner (<https://exifcleaner.com/>) that allows viewing and/or removing those properties
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^324] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It is however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^325] and available on TAILS by default[^326].
...
Note that it's possible to run Whonix in Live mode leaving no traces when you shut down the VMs, consider reading their documentation here <https://www.whonix.org/wiki/VM_Live_Mode> and here <https://www.whonix.org/wiki/Warning#Whonix_.E2.84.A2_Persistence_vs_Live_vs_Amnesic>.
...
Many of those can be deleted using some various commercial third-party tools but I would personally recommend using the free and well-known Onyx which you can find here: <https://www.titanium-software.fr/en/onyx.html>. Unfortunately, it is closed-source but it is notarized, signed and has been trusted for many years.
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^241]), your network connections, app crash information and various errors. It is always best to clean those up regularly. Do not do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally, I would just pick Normal Deletion + Trim (Trim itself should be enough[^245]). Secure Deletion with Trim[^242] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^327] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^328] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^329] and Secure Communication Obfuscation[^330] through Steganography[^331] and/or Kleptography[^332] and has many names such as Koalang[^333] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It is hiding in plain sight.
...
Draw a line with your cable and place objects along the line. When you are back, just check those places and check if the objects are still placed along the line. This allows you not to remember precisely where your things were.
...
-   **Remember to use passphrases instead of passwords and use a different one for each service**[^212]**.**
...
-   Encrypt everything but do not take it as granted. Remember the 5\$ wrench[^7] .

-   Keep plausible deniability as an option but remember it will not help against the 5\$ wrench either[^7].
...
-   If you can, do not talk to the police (at least if you are in the US) <https://www.youtube.com/watch?v=d-7o9xYp7eE> without a lawyer.
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> [^334]
...
As written earlier in this guide and as noted by Privacytools.io[^335], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple of utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   You are done, if you need to restore, follow these instructions: <https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/02_Restore_disk_image>
...
If you are experiencing issues connecting to Tor due to Censorship or Blocking, you might consider using Tor bridges as explained here: <https://bridges.torproject.org/>
...
-   The Windows USN journal which stores plenty of information[^337].

-   The Windows System Resource Usage Monitor (SRUM)[^338].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^339].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^340]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
Some might be very efficient[^341] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It is pretty much the only way to be sure it is not only supported but actually works. Some utilities can tell you if it is supported or not like CrystalDiskInfo[^342] but will not actually check if it is working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives:]
...
**It is not "recommended". Please read the disclaimers here <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase> and here <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing>**
...
According to Apple Documentation[^322], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
If your disk supports it but it is not enabled in MacOS. You could try issuing a "sudo trimforce enable" command from the Terminal and see if it enables Trim on your external SSD. And then again check the first aid command if it is not APFS (see this Tutorial for info <https://www.lifewire.com/enable-trim-for-ssd-in-os-x-yosemite-2260789>)

If it does not work, I am not aware of any reliable method to enable TRIM besides the commercial utility Trim Enabler here <https://cindori.org/trimenabler/> which claims support for external drives.
...
-   Put an IP in the Whonix range like for example 10.152.152.55 (do not use the same IP as any other VM you put behind the Whonix Gateway)
...
-   After updates, shutdown, take a new snapshot, remove the previous one keeps going
...
-   If you really must take them with you, you could consider the use of a faraday cage[^343] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^344] for their effectiveness. If you cannot afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
Personally, I would recommend Mullvad and iVPN due to personal experience.

**I would not recommend ProtonVPN because they do require an e-mail for registration unlike Mullvad and iVPN.**
...
-   Wearing gloves while manipulating anything to avoid leaving fingerprints[^345] and touch DNA[^346].
...
-   Do not lick the envelope or the stamps[^347] if you use them to avoid leaving DNA traces.
...
There might be situations where simply using Tor or a VPN alone could be suspicious and could be dangerous for your safety. If this is case, you could be on a very hostile environment where surveillance and control is high. Consider reading this guide <https://archive.flossmanuals.net/bypassing-censorship/index.html>
...
**Personally, I would recommend the AWUS036 series in the Alfa brand of adapters (see <https://www.alfa.com.tw/>).** But you could also go with some other brands if you want such as the TP-Link TL-WN722 (see <https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/>).
...
So, what is OONI? OONI stands for Open Observatory of Network Interference and is a sub-project of the Tor Project[^204].
...
The problem obviously is that your network providers will be able to see those tests and your attempts at connecting to various services if the network is monitored. The other issue is that there are solutions to prevent OONI from working properly[^348].
...
Usually, integrity checks[^349] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^350], more recently MD5[^351] but those present several weaknesses (CRC, MD5[^352]) that makes them unreliable for file integrity checks (which does not mean they are not still widely used in other contexts).

This is because they do not prevent Collision[^353] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having a different content.

For this reason, it is usually recommended to use SHA[^354] based hashes and the most used is probably the SHA-2[^355] based SHA256 for verifying file integrity. SHA is much more resistant to collisions than CRC and MD5.
...
Integrity is one thing. Authenticity is another thing. This is a process where you can verify some information is authentic and from the expected source. This usually done by signing information (using GPG[^358] for instance) using public key cryptography[^359].
...
-   <https://tails.boum.org/install/vm-download/index.en.html> (See Basic OpenPGP verification).
...
**Every check should happen in sandboxes and hardened Virtual Machines. This is to prevent any malware from accessing your computer. Do not perform these from the Host OS unless you are using TAILS.**

Fortunately, most recent anti-virus programs should be able to protect you from most of the known threats. Yet if you are in doubt. I encourage you to check any **[non-sensitive]{.ul}** documents/images/videos/archives/programs you intend to open at <https://www.virustotal.com/gui/> (just upload the file and wait for the results).
...
**Be aware that any document you submit to them will be kept, shared, and used commercially including the content. So, you should not do that with sensitive information and rely on various local AV scanners (that do not send samples online).**

This guide's PDF files were submitted to them because it is meant to be public knowledge.
...
/XFA 0 \#This indicates the presence of XML Forms within the PDF 0

```

For other formats, please refer to:

-   This practical cheat sheet from SANS: <https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf> (warning, many of those tools will be harder to use to Windows and you might consider using them from a Linux OS such as Tails, Whonix Workstation or a Linux distribution of your choice as explained later in this guide. There are also other guides out there[^360] that might be of use.

-   This GitHub repository with various resources on malware analysis: <https://github.com/rshipp/awesome-malware-analysis>

-   This interesting PDF with various links <https://www.winitor.com/pdf/Malware-Analysis-Fundamentals-Files-Tools.pdf>

**Even with all those resources, keep in mind you might still get advanced malware if those are not detected by those various tools. Be careful and remember to handle these files within Virtual Machines if possible.**
...
There are plenty of methods you could use to bypass those restrictions locally. One of them would be to use portable apps[^361]. Those apps do not require installation your system and can be run from an USB key or anywhere else.
...
If you are to resort to this, you should never ever do so from a monitored/known network but only from a safe different network. Ideally a safe public wi-fi (see [Find some safe places with decent public WIFI:]).
...
[^19]: Wikipedia, Real-Name System Germany <https://en.wikipedia.org/wiki/Real-name_system#Germany>

[^20]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil>

[^21]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address>

[^22]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention>

[^23]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)>

[^24]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network>

[^25]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System>

[^26]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking>

[^27]: CensoredPlanet <https://censoredplanet.org/>

[^28]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf>

[^29]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole>

[^30]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS>

[^31]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS>

[^32]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole>

[^33]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication>

[^34]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^35]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^36]: Mozilla Blog, Encrypted Client Hello: the future of ESNI in Firefox <https://blog.mozilla.org/security/2021/01/07/encrypted-client-hello-the-future-of-esni-in-firefox/>

[^37]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf>

[^38]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_network>

[^39]: Cloudflare, Good-bye ESNI, hello ECH! <https://blog.cloudflare.com/encrypted-client-hello/>

[^40]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/>

[^41]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/>

[^42]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf>

[^43]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries>

[^44]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/>

[^45]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity>

[^46]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity>

[^47]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers>

[^48]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US>

[^49]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law>

[^50]: Bellingcat, The GRU Globetrotters: Mission London <https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/>

[^51]: Bellingcat,\"V\" For "Vympel": FSB's Secretive Department "V" Behind Assassination Of Georgian Asylum Seeker In Germany <https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/>

[^52]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television>

[^53]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html>

[^54]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/>

[^55]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher>

[^56]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker>

[^57]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous \'Stingrays\' Become \'Obsolete\' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778>

[^58]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack>

[^59]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/>
...
[^89]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^90]: Apple Support <https://support.apple.com/en-us/HT210515>

[^91]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/>

[^92]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756>

[^93]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy>

[^94]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^95]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification>

[^96]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication>

[^97]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/>

[^98]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/>

[^99]: Zoom Support, Adding a watermark <https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark>

[^100]: Zoom Support, Audio Watermark <https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark>

[^101]: CreativeCloud Extension, IMATAG <https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html>

[^102]: NexGuard, <https://dtv.nagra.com/nexguard-forensic-watermarking>

[^103]: Vobile Solutions, <https://www.vobilegroup.com/solutions>

[^104]: Cinavia, <https://www.cinavia.com/languages/english/pages/technology.html>

[^105]: Imatag, <https://www.imatag.com/>

[^106]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^107]: IEEExplore, A JPEG compression resistant steganography scheme for raster graphics images <https://ieeexplore.ieee.org/document/4428921>

[^108]: ScienceDirect, Robust audio watermarking using perceptual masking <https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140>

[^109]: IEEExplore, Spread-spectrum watermarking of audio signals <https://ieeexplore.ieee.org/abstract/document/1188746>

[^110]: Google Scholar, source camera identification <https://scholar.google.com/scholar?q=source+camera+identification>

[^111]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html>

[^112]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/>

[^113]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/>

[^114]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/>

[^115]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/>

[^116]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117>

[^117]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524>

[^118]: Bellingcat, How To Track Illegal Funding Campaigns Via Cryptocurrency, <https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/>

[^119]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer>

[^120]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y>

[^121]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/>

[^122]: Wikipedia, Cryptocurrency Tumbler <https://en.wikipedia.org/wiki/Cryptocurrency_tumbler>

[^123]: Wikipedia, Security Through Obscurity <https://en.wikipedia.org/wiki/Security_through_obscurity>

[^124]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^125]: SSRN, The Cryptocurrency Tumblers: Risks, Legality and Oversight <https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361>

[^126]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)>

[^127]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting>

[^128]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/>

[^129]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach>

[^130]: BBC, China social media: WeChat and the Surveillance State <https://www.bbc.com/news/blogs-china-blog-48552907>

[^131]: The Intercept, Revealed: Massive Chinese Police Database <https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/>

[^132]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)>

[^133]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit>

[^134]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine>

[^135]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor>

[^136]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities>

[^137]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor>

[^138]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME>

[^139]: Libreboot, <https://libreboot.org/>

[^140]: Digi.Ninja, Jasager <https://digi.ninja/jasager/>

[^141]: Hak5 Shop, Wi-Fi Pineapple <https://shop.hak5.org/products/wifi-pineapple>

[^142]: Wikipedia, Deautentication Attack <https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack>

[^143]: Wikipedia, Capture Portal <https://en.wikipedia.org/wiki/Captive_portal>

[^144]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/>

[^145]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet>

[^146]: Superuser.com, How do I safely investigate a USB stick found in the parking lot at work? <https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work>

[^147]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/>

[^148]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/>

[^149]: ArsTechnica, How the way you type can shatter anonymity---even on Tor <https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/>

[^150]: Paul Moore Blog, Behavioral Profiling: The password you can\'t change. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/>

[^151]: EFF CoverYourTracks, <https://coveryourtracks.eff.org/>

[^152]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>

[^153]: Bellingcat MH17 - Russian GRU Commander 'Orion' Identified as Oleg Ivannikov <https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/>

[^154]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^155]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^156]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^157]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^158]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>

[^159]: MIT Technology Review, This is how we lost control of our faces, <https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/>

[^160]: Bellingcat, Shadow of a Doubt: Crowdsourcing Time Verification of the MH17 Missile Launch Photo <https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/>

[^161]: Brown Institute, Open-Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^162]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^163]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^164]: APNews, Chinese \'gait recognition\' tech IDs people by how they walk <https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a>

[^165]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/>

[^166]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^167]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^168]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^169]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^170]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^171]: Anyvision <https://www.anyvision.co/>

[^172]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^173]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^174]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^175]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/>

[^176]: BBC, WhatsApp photo drug dealer caught by \'groundbreaking\' work <https://www.bbc.com/news/uk-wales-43711477>

[^177]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^178]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^179]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^180]: Wikipedia, Deepfake <https://en.wikipedia.org/wiki/Deepfake>

[^181]: Econotimes, Deepfake Voice Technology: The Good. The Bad. The Future <https://www.econotimes.com/Deepfake-Voice-Technology-The-Good-The-Bad-The-Future-1601278>

[^182]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events>

[^183]: Forbes, A Voice Deepfake Was Used To Scam A CEO Out Of \$243,000 <https://www.forbes.com/sites/jessedamiani/2019/09/03/a-voice-deepfake-was-used-to-scam-a-ceo-out-of-243000/>

[^184]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^185]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^186]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^187]: Wikipedia, Social Engineering <https://en.wikipedia.org/wiki/Social_engineering_(security)>

[^188]: Grayshirt, <https://www.grayshift.com/>

[^189]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf>

[^190]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^191]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^192]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^193]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^194]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^195]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^196]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^197]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^198]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^199]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^200]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^201]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone>

[^202]: Wikipedia, Rubber-hose Cryptanalysis https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis

[^203]: Defuse.ca, TrueCrypt\'s Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm>

[^204]: Wikipedia, OONI, <https://en.wikipedia.org/wiki/OONI>

[^205]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^206]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^207]: Usenix.org, Shedding too much Light on a Microcontroller's Firmware Protection <https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf>

[^208]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^209]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^210]: Coreboot, <https://www.coreboot.org/>

[^211]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid>

[^212]: XKCD, Password Strength <https://xkcd.com/936/>

[^213]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^214]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^215]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^216]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^217]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^218]: Sneak.Berlin, Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^219]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^220]: Wikipedia, APNS <https://en.wikipedia.org/wiki/Apple_Push_Notification_service>

[^221]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^222]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^223]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption>

[^224]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^225]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/>

[^226]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^227]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^228]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^229]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^230]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^231]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^232]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^233]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^234]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^235]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^236]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^237]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^238]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^239]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^240]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^241]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^242]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^243]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^244]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^245]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^246]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^247]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^248]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^249]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^250]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^251]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^252]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987>

[^253]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox>

[^254]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^255]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^256]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^257]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^258]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>

[^259]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^260]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^261]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^262]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^263]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^264]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^265]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^266]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^267]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^268]: hCaptcha <https://www.hcaptcha.com/>

[^269]: hCaptcha, hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^270]: Nearcyan.com, You (probably) don't need ReCAPTCHA <https://nearcyan.com/you-probably-dont-need-recaptcha/>

[^271]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^272]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^273]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^274]: Tor Project Community, Cloudflare Captcha Monitoring <https://community.torproject.org/gsoc/cloudflare-captcha-monitoring/>

[^275]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^276]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^277]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^278]: Developers Google Blog,
...
[^279]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^280]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^281]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^282]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^283]: DigitalInformationWorld, Facebook will now require you to Create a Video Selfie for Identity Verification <https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html>

[^284]: Vice.com, PornHub Announces \'Biometric Technology\' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id>

[^285]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify>

[^286]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^287]: Wikipedia, Sim Swapping <https://en.wikipedia.org/wiki/SIM_swap_scam>

[^288]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^289]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^290]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^291]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^292]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^293]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^294]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^295]: Google Help, Update your account to meet age requirements <https://support.google.com/accounts/answer/1333913?hl=en>

[^296]: Jumio, ID verification features <https://www.jumio.com/features/>

[^297]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^298]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^299]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^300]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^301]: Privacytools.io, File-Sharing <https://privacytools.io/software/file-sharing/>

[^302]: Privacytools.io, Real-Time Communication <https://privacytools.io/software/real-time-communication/>

[^303]: Praxis Films, Open Letter from Laura Poitras <https://www.praxisfilms.org/open-letter-from-laura-poitras/>

[^304]: Wikipedia, SecureDrop <https://en.wikipedia.org/wiki/SecureDrop>

[^305]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^306]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^307]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^308]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^309]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^310]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^311]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^312]: ResearchGate, Live forensics method for acquisition on the Solid-State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^313]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^314]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^315]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^316]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^317]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^318]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli>

[^319]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/>

[^320]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/>

[^321]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^322]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html>

[^323]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^324]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^325]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^326]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^327]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^328]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^329]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^330]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^331]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^332]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^333]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^334]: Tor Documentation, Snowflake, <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/>

[^335]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^336]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^337]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^338]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^339]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^340]: DBAN, <https://dban.org/>

[^341]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^342]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/>

[^343]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^344]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^345]: YouTube, Fingerprinting Paper - Forensic Education <https://www.youtube.com/watch?v=sO98kDLkh-M>

[^346]: Wikipedia, Touch DNA, <https://en.wikipedia.org/wiki/Touch_DNA>

[^347]: TheDNAGuide, DNA from Postage Stamps or Hair Samples? Yeeesssss\..... <https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps>

[^348]: GitHub, Mhinkie, OONI-Detection <https://github.com/mhinkie/ooni-detection>

[^349]: Wikipedia, File Verification <https://en.wikipedia.org/wiki/File_verification>

[^350]: Wikipedia, CRC <https://en.wikipedia.org/wiki/Cyclic_redundancy_check>

[^351]: Wikipedia, MD5 <https://en.wikipedia.org/wiki/MD5>

[^352]: Wikipedia, MD5 Security <https://en.wikipedia.org/wiki/MD5#Security>

[^353]: Wikipedia, Collision Resistance <https://en.wikipedia.org/wiki/Collision_resistance>

[^354]: Wikipedia, SHA <https://en.wikipedia.org/wiki/Secure_Hash_Algorithms>

[^355]: Wikipedia, SHA-2 <https://en.wikipedia.org/wiki/SHA-2>

[^356]: GnuPG Gpg4win Wiki, Check integrity of Gpg4win packages <https://wiki.gnupg.org/Gpg4win/CheckIntegrity>

[^357]: Medium.com, How to verify checksum on Mac <https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f>

[^358]: Wikipedia, GPG <https://en.wikipedia.org/wiki/GNU_Privacy_Guard>

[^359]: Wikipedia, Public-Key Cryptography <https://en.wikipedia.org/wiki/Public-key_cryptography>

[^360]: Lenny Zeltser, Analyzing Malicious Documents Cheat Sheet <https://zeltser.com/analyzing-malicious-documents/>

[^361]: Wikipedia, Portable Applications <https://en.wikipedia.org/wiki/Portable_application>
  ...
  [Appendix Q: How to bypass (some) local restrictions on supervised computers]: #appendix-q-how-to-bypass-some-local-restrictions-on-supervised-computers
  [Portable Apps:]: #portable-apps
  [Bootable Live Systems:]: #bootable-live-systems
  [Precautions:]: #precautions
  [Acknowledgements:]: #acknowledgements
  [cc-by-4.0]: https://creativecommons.org/licenses/by/4.0/
  [21]: media/image1.jpeg
  [22]: media/image2.jpeg 
  [23]: media/image3.jpeg
  [24]: media/image4.jpeg 
  [www.pornhub.com]: http://www.pornhub.com
  [25]: Anonymized#_Your_
  [26]: media/image5.jpeg
  [27]: media/image6.jpeg
  [28]: media/image7.jpeg
  [29]: media/image8.jpeg
  [30]: media/image9.jpeg 
  [31]: media/image10.jpeg
  [32]: media/image11.jpeg
  [33]: media/image12.jpeg
  [34]: media/image13.jpeg 
  [35]: media/image14.jpeg 
  [36]: media/image15.jpeg
  [37]: media/image16.jpeg 
  [38]: media/image17.jpeg
  [39]: media/image18.jpeg
  [40]: media/image19.jpeg
  [41]: media/image20.jpeg 
  [42]: media/image21.jpeg
  [43]: media/image22.jpeg
  [44]: media/image23.jpeg
  [45]: media/image24.jpeg
  ...
  [46]: media/image25.jpeg
  [47]: media/image26.jpeg
  [48]: media/image27.jpeg
  [49]: media/image28.jpeg
  [50]: media/image29.jpeg
  [51]: media/image30.jpeg
  [Online Phone Number (less recommended):]: #online-phone-number-less-recommended
  [52]: media/image31.jpeg 
  [53]: media/image32.jpeg
  [54]: media/image33.jpeg
  [55]: media/image34.jpeg
  [56]: media/image35.jpeg
  [57]: media/image36.jpeg
  [Extra Tools Cleaning:]: #extra-tools-cleaning
  [58]: media/image37.jpeg
  [59]: media/image38.jpeg
  [60]: media/image39.jpeg
  [61]: media/image40.jpeg
