...
Version 0.7.7 (draft), February 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
"bc1qekuxlepa52j40ntwj7rrga823d9v0rd4zzwtwz" **(please do verify the checksum and gpg signature of this file for authenticity, this is explained in the README of the repository if you don't know how to do that)**.
...
Apple devices are IMHO among the best choices for good Privacy but not necessarily for Anonymity. They will protect people from others knowing what you're doing but not protect you from others (Apple themselves for instance) knowing who you are.
...
DNS stands for "Domain Name System"[^25] and is a service used by your browser (and other apps) to find the IP addresses of a service. It's pretty much a huge "contact list" (phone book for older people) that works like asking it a name and it returns the number to call. Except it returns an IP instead.
...
Here is a video explaining DNS visually if you're already lost: <https://www.youtube.com/watch?v=vrxwXXytEuI>
...
Unfortunately, the TLS protocol used in most HTTPS connections in most Browsers (Chrome/Brave among them) will leak the DNS again through SNI[^33] handshakes (this can be checked here at Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> ). **As of the writing of this guide**, **only Firefox based browsers supports eSNI (encrypted SNI**[^34] **soon to be renamed ECH**[^35] **and soon to be supported by Firefox too**[^36]**) which will encrypt everything end to end (in addition to using a secure private DNS over TLS/HTTPS) and will allow you to actually hide your DNS requests from a third party**[^37]**.** And this option is not enabled by default either so you'll have to enable it yourself:
...
Some countries like Russia[^40] and China[^41] will block eSNI handshakes at network level to allow snooping and prevent bypassing censorship. Meaning you won't be able to establish an HTTPS connection with a service if you don't allow them to see what it was.
...
Lastly, there is also this new option called DoHoT which stands for DNS over HTTPS over Tor which could also further increase your privacy/anonymity and which you could consider if you are more skilled with Linux. See <https://github.com/alecmuffett/dohot>. This guide won't help you with this one at this stage but it might be coming soon.
...
-   Government agencies around the world interested in your phone number can and do use[^54] special devices called "IMSI catchers"[^55] like the Stingray[^56] or more recently the Nyxcell[^57]. These devices are able to impersonate (to spoof) a cell phone Antenna and force a specific IMSI (your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^58] (Man-In-The-Middle Attacks) that will allow them to:
...
Such devices will continue to broadcast identity information to nearby devices even when offline using Bluetooth Low-Energy[^93]. They don't have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^94].
...
Lastly, do remember that using Tor in itself can already be considered a suspicious activity[^115] and its use could be considered malicious by some[^116].
...
This doesn't mean you can't use Bitcoin anonymously at all. You can actually use Bitcoin anonymously as long as you don't convert it to actual currency and use a Bitcoin wallet from a safe anonymous network. Meaning you should avoid KYC regulations by various exchanges and avoid using the Bitcoin network from any known IP address. This also means you'll not be able to convert these Bitcoin from/to real currency easily and without involving some risks.

**All in all, IMHO, the best option for using Crypto with reasonable anonymity and privacy is still Monero and you should ideally not use any other for sensitive transactions unless you are aware of the limitations.**

## Malware and Exploits in your apps and services:

So, you're using Tor Browser or Brave Browser over Tor. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^126] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.
...
If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^139] or Coreboot[^210] if your laptop supports it.
...
Some of these can be avoided by the use of Virtualization Software settings that can mitigate such exploits. See this guide for more information <https://www.whonix.org/wiki/Spectre_Meltdown> (warning: these can severely impact the performance of your VMs).
...
These devices can then mimic a captive portal[^143] with the exact same layout as the Wi-Fi you're trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you open access internet that they'll themselves get from the same place.
...
"Hell is other people", even if you evade every method listed above, you're not out of the woods yet thanks to the widespread use of advanced Face recognition by everyone.
...
Even if you're not looking at the camera, they can still figure out who you are[^162], make out your emotions[^163], analyze your gait[^164] and probably guess your political affiliation[^165].
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they're encrypted[^188]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And last but not least here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^189]
...
Last but not least, it's now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^198] and Omnisec story[^199]).
...
This is the hardest part of the whole guide. It's a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting more and more limited due to various KYC type regulations[^205].
...
TAILS[^208] stands for **The Amnesic Incognito Live System**. It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
You might also wonder if this will result in a "Tor over Tor" setup but it won't. The Whonix VMs will be accessing the network directly and not through TAILS Onion Routing.
...
-   Go into Utilities and Unlock your Veracrypt (hidden) Volume (don't forget to check the hidden volume checkbox)
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. However, if you're going for the Qubes Route consider Intel as they do not support AMD with their anti-evil-maid system[^211].
...
So, what is Secure Boot[^214]? In short, it's a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.
...
This route will make extensive use of Virtual Machines[^217], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
So, what is Plausible Deniability? It's the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^223].
...
Cold Boot attacks[^228] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.
...
-   Encrypt the memory with a Veracrypt option[^240] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   To rescue disk[^250] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
-   Connect to VirtWifi Wi-Fi Network **(see the above section if you're behind Whonix and want to use Tor)**

You're now done and can now install any Android app.
...
Since the x86 Android doesn't work "well" with Qubes OS. I will instead recommend using AnBox.io which works "well enough" with Qubes OS.
...
Now you'll have to follow the instructions from here: <https://github.com/anbox/anbox-modules>
...
If you frequently use VPNs or Tor, you'll quickly encounter many captchas everywhere[^270]. Quite often when using Tor, even if you succeed in solving all the puzzles, you'll still be denied after solving the puzzles.

While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^271]:
...
-   They track your behavior before/during/after the tests to ensure you're "human"[^273].
...
You will often experience several in a row (sometimes endless[^274]) and sometimes very difficult ones involving reading undecipherable characters or identifying various objects on endless pictures set. You'll also have more captchas if you use ad blocking system or if your account was flagged for any reason for using VPNs or Tor previously.
...
Many platforms (like Google[^278]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^282]. Unfortunately, this method is now also sometimes being used on Facebook[^283] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
These can be triggered by any of the above and just means someone (usually specialized employees) will review your profile manually and decide if it's real or not based on their subjective opinion.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^286]).
...
-   "Some animals are more equal than others".
    ...
    -   Ethnicity is important and you will have less issues and attract less attention to verification algorithms if your identity is Caucasian/East-Asian than if it's Arabic/Black (yes, I tested this extensively and it's definitely an issue).

    -   Age is important and you will have less issues if you're young (18-22) than if you're middle-aged or older. Platforms seem to be more lenient in not imposing restrictions on new younger audiences.

    -   Sex/Gender is important and you'll have fewer issues if you're a female than if you're a male.

    -   Country of origin is important and you'll have fewer issues if your identity is Norwegian than if it's Ukrainian, Nigerian or Mexican.
    ...
    -   Language is important and you'll have fewer issues if you speak English or the language of your Identity than if you use a non-related language. Don't make a Norwegian born Arabic 20-year-old female that speaks Ukrainian or Arabic.
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will definitely have less issues with captchas and online platforms[^278] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Do not pick an occupation at a well-known private corporations/company as they have people in their HR departments monitoring activities in platforms such as LinkedIn and will report your profile as being fake if it doesn't match their database. Instead pick an occupation as a freelancer or at a very large public institution where you'll face less scrutiny due to their decentralized nature.
...
This doesn't mean that it's illegal in other places but that it might be a breach of their Terms of Services if you don't have the law on your side. **Remember this guide only endorses this for German users residing in Germany.**
...
Alternatively, you could be an Adult resident of any other country where you can validate and verify the legality of this yourself. Again, this is not legal advice and I'm not a lawyer. **Do this at your own risk.**
...
-   "Likely": It did happen in a majority of my tests.
...
-   "Medium": The overall experience has some obstacles but it's still doable without too much hassle.

-   "Hard": The overall experience is a painful struggle with many obstacles.

-   "N/A": Not Applicable because it wasn't possible to test within the context of this guide

-   "Indirectly": This means they do require something but indirectly through a third-party system (Financial KYC for example).

| **Service** | **Against ToS** | **Requires Phone** | **Requires E-Mail** | **VPN Sign-up** | **Tor Sign-up** | **Captchas** | **ID Checks**  | **Facial Checks** | **Manual Checks** | **Overall difficulty** |
|-------------|-----------------|--------------------|---------------------|-----------------|-----------------|--------------|----------------|-------------------|-------------------|------------------------|
| Amazon      | No              | No                 | Yes                 | Yes             | Yes             | No           | **Indirectly** | No                | Unclear           | **N/A**                |
...
| Element     | No              | No                 | No                  | Yes             | Yes             | No           | No             | No                | No                | Easy                   |
...
While it doesn't technically require a real-name. It does require a valid payment method. Unfortunately, it won't accept "cash" or "Monero" as a payment method. So instead, they're relying on financial KYC (where a real-name policy is pretty much enforced everywhere).

-   Will they require a phone number? No

-   Can you create accounts through Tor? Yes

Because of this valid payment method requirement, I couldn't test this. While this is seemingly not against their ToS, it's not really possible within the context of this guide unless you manage to obtain a valid KYC payment method anonymously which AFAIK is pretty much impossible or extremely difficult.
...
-   Will they require a phone number? No, they don't even require an e-mail
...
-   Can you create accounts through Tor? Yes, but it's very difficult and their onion address[^290] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in."
...
ProtonMail is good ... but to appear less suspicious, it's just better to also have a mainstream Google Mail account.
...
It's also possible that Google will require an ID check through indirect financial KYC or ID picture check if you attempt to access/publish mature content on their platform[^295].
...
-   Is this against their ToS? **Maybe?** I'm not sure <https://help.instagram.com/581066165581870?ref=dp>
...
This one is a bit of an Oxymoron don't you think? So, I'm not sure if it's allowed or not.
...
-   Is this against their ToS? No unless it's about crypto <https://policy.medium.com/medium-terms-of-service-9db0094a1e0f>
...
-   Is this against their ToS? No, they don't even have Terms of Services
...
They're recommended by privacytools.io[^297]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^298]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the phone number, you should be reasonably safe from tracking.
...
This time you will need to submit an appeal using a form[^299], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
Note that you won't be able to enable 2FA on Twitch using only e-mail. This feature requires a phone number to enable.
...
4chan is 4chan ... This guide won't explain 4chan to you. They block Tor exit nodes and known VPN IP ranges.

You're going to have to find a different way to post there using at least seven proxies[^300] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
-   (Not recommended and most complicated) Have a burner Smartphone that you will connect to the VM layered network through Tethering/Sharing of the connection through Wi-Fi. I won't detail this here but it's an option if you really want to.
...
For this purpose, I won't recommend classic messengers like Signal/Telegram as those require a phone number.
...
All these options won't require any identifiable information.

### Communicating sensitive information to various known organizations:

You might be interested in communicating information to some organization such as the press anonymously.

If you have to do so, you should take some steps because you can't really trust any organization to protect your anonymity[^303]:
...
So, the fact is that it's very unlikely[^311]'[^312] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it's not completely impossible either[^313]'[^314]'[^315] if they're fast enough and have access to extensive equipment, skills and motivation.
...
-   If your file system is APFS, you don't need to worry about Trim, it apparently happens asynchronously as the OS writes data[^322] according to their own documentation.
...
Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^323]. So, you're left with using other tools:
...
It's actually simple, jut install exiftool and run:
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^324] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It's however very straightforward to install and use on Linux.
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^241]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
Now it's time to clear the history of the Wi-Fi you connect to. Unfortunately, Windows keeps storing a list of past Networks in the registry even if you "forgot" those in the Wi-Fi settings. As far as I know, no utilities clean those yet (BleachBit or PrivaZer for instance) so you'll have to do it the manual way:
...
**Use the manufacturer provided tools to check and perform these operations** ... It's pretty much the only way to be sure it's not only supported but actually works. Some utilities can tell you if it's supported or not like CrystalDiskInfo[^342] but won't actually check if it's working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives:]
...
-   If you really have to take them with you, you could consider the use of a faraday cage[^343] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^344] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
-   Don't lick the envelope or the stamps[^347] if you use them to avoid leaving DNA traces.
...
Usually, integrity checks[^349] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^350], more recently MD5[^351] but those present several weaknesses (CRC, MD5[^352]) that makes them unreliable for file integrity checks (which doesn't mean they're not still widely used in other contexts).

This is because they don't prevent Collision[^353] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having a different content.

For this reason, it's usually recommended to use SHA[^354] based hashes and the most commonly used is probably the SHA-2[^355] based SHA256 for verifying file integrity. SHA is much more resistant to collisions than CRC and MD5.
...
Obviously, this checksum should itself be authenticated/trusted and should be available from an authenticated/trusted source (obviously you shouldn't trust a file just because it has a checksum attached to it alone).
...
**Remember that checksums are just checksums. Having a matching checksum doesn't mean the file is safe.**
...
-   Download the Signature key from a trusted source. If someone is not giving you a key directly, you should check for multiple versions on other websites to confirm you're using the right key (GitHub, GitLab, Twitter, Keybase, Public Keys Servers...).
...
All of these guides should also apply to any other file with any other key.

## Security (checking for actual malware):

**Every check should happen in sandboxes and hardened Virtual Machines. This is to prevent any malware for actually accessing your computer. Do not perform these from the Host OS unless you're using TAILS.**
